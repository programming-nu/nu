...
(class NuConsoleShowHideTransformer is NSValueTransformer
     ...
     (imethod (id) transformedValue: (id) v is
          (if v
              (then "Hide Nu Console")
              (else "Show Nu Console"))))
...
(class NuConsoleWindowController is NSWindowController
     ...
     (imethod (void) addMenuItem is
          (let (m ((NSMenuItem alloc) initWithTitle:"Toggle Nu Console" action:"toggleConsole:" keyEquivalent:"l"))
               ...
               (set windowMenu (((NSApplication sharedApplication) mainMenu) onlyMatchingChild:
                                (do (x) (and (eq (x className) "NSMenu") (eq (x title) "Window")))))
               (windowMenu insertItem:m atIndex:0)))
     
     ;; Initialize a console.
     (imethod (id) init is
          (set frame '(0 0 600 200))
          (set styleMask (+ NSTitledWindowMask NSClosableWindowMask NSMiniaturizableWindowMask NSResizableWindowMask NSUtilityWindowMask))
          (self initWithWindow:((NSPanel alloc) initWithContentRect:frame styleMask:styleMask backing:NSBackingStoreBuffered defer:0))
          (set @console ((NuConsoleViewController alloc) initWithFrame: (list 0 0 (- (third frame) 17) (fourth frame))))
          (self setMyShowConsole:NO)
          (self addMenuItem)
          (set @exitWhenClosed NO)
          (let (w (self window))
               (w setContentView: (make-scrollview '(0 0 600 200)
...
(class NuConsoleViewController is NSObject
     ...
     (imethod (void) prompt is
          ...
          (set savedInsertionPoint @insertionPoint)
     ...
     (imethod (void) write: (id) string is
          ...
          (set stringLength (string length))
          (set @insertionPoint (+ @insertionPoint stringLength))
          (set @startOfInput (+ @startOfInput stringLength))
     ...
     (imethod (id) currentLine is
          (set text ((@textview textStorage) mutableString))
     ...
     (imethod (void) replaceLineWithPrevious is
          (set range (list @startOfInput (- (self lengthOfTextView) @startOfInput)))
          (cond ((eq @index 0) nil)
                ((eq @index -1) nil)
                (t
                  (set @index (- @index 1))
                  (set newstring (@history objectAtIndex: @index))
                  ((@textview textStorage) replaceCharactersInRange:range withString:newstring)
                  (@textview scrollRangeToVisible:(list (self lengthOfTextView) 0)))))
